/**
 * Staff top bar: project filter, GlobalSearch, notifications, sign-out.
 */
...
export default function ConsoleTopbar({ onMenu }: { onMenu: () => void }) {
  const { user } = useAuth();
  ...
  return (
    <header className="sticky top-0 z-20 h-16 border-b border-background-200 bg-background-50/95 backdrop-blur">
      <div className="flex h-full items-center gap-3 px-4 md:px-6">
        ...
        <GlobalSearch mode="console" placeholder="Search tickets, projects, people…" />
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="flex items-center gap-2.5 pl-2 ml-1 border-l border-background-200">
            ...
            <div className="hidden md:block leading-tight">
              <p className="text-sm font-semibold text-foreground-950">{user?.full_name ?? "Siyana staff"}</p>
              <p className="text-[11px] text-foreground-500">{user?.role}</p>
            </div>
          </div>
        </div>
      </div>
